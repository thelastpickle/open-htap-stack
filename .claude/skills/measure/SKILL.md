...
Per engine the response carries `query_time_ms`, `row_count`, `rows`, `available`, `error`, the sampled `oltp`, and for the bulk reader `snapshot_bytes`, `snapshot_ms`, `snapshot_reused`, `snapshot_age_s`.  **`query_time_ms` already contains `snapshot_ms`**, because `SparkBulkPath` prepares the snapshot inside the call being timed; adding the two overstates the read, and doing so failed a CI assertion by claiming 2830 ms of work for a path whose 2427 ms was honest.  The cqlite path takes no snapshot and reports four fields of its own instead: `sstable_files` and `sstable_bytes` for the live files it merged, `reader_open_ms` for what listing and opening them cost, and `data_age_s` for how long ago the newest of them was written.  The top level adds `mode`, `oltp_baseline`, `oltp_combined` and `cancelled`.  Read the field names from the records in `htap-backend`'s `api/dto` package, `EngineResult` for these, rather than guessing them; `duration_ms` and `oltp_impact` are plausible and wrong.  The names are snake_case on the wire and camelCase in the records, which one Jackson setting decides, so a record component is the name to read and not the string to expect.
...
**`memory.peak` is not the reader's memory.**  A cgroup's peak sums anonymous memory and page cache, and the cqlite path reads files, so it charges what it reads to the backend's container: a walk of the whole table peaked at 4.37 GB while holding 0.15 GB of its own, the other 4.19 GB being SSTable pages the kernel reclaims rather than kills for.  Reading the peak alone said a streaming walk held the table, which is the opposite of what that merger does.  Sample `anon` from `memory.stat` while the statement runs — `scripts/cqlite-memsample.sh <label> "<sql>" [interval_s]` — and recreate the backend first, because both figures are high-water marks that outlive a query.  A sample costs two `podman exec` calls: at one per second it stretched a 24 s statement to 58 s, so take a wall clock from an unsampled run.
