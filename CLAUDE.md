...
**The reader applies the token bound itself, because cqlite drops it.** `ScanTokenBound` is documented as a pushdown hint the consumer must enforce, and a Cassandra 5 generation is BTI, which fails the gate on cqlite's Summary-guided walk and drains through the stitch-and-parse window instead; that route takes no bound.  So `scan.rs::in_slice` tests the partition's token at the partition boundary.  Measured before the test: four slices over a 100-row table returned 400 rows.  After it: 100 rows at 1, 2, 3, 7 and 16 slices, agreeing with CQL.  The slices cost what the comment claims, too — 34.1 ms, 69.7 ms and 105.8 ms for one, two and three slices of the same table, because each slice reads the whole ring and only the decode divides.

**`cqlite_splits` stays at 1 because memory binds long before throughput does.** One 15-minute window counted through the seek path peaked at 6.77 GB of anonymous resident memory on a single slice; a repeat of the same statement reached 8.95 GB, and the same window with ten columns projected twice took the backend to a global out-of-memory kill, at 11.96 GB and 13.06 GB.  N slices build N mergers over the same files, so raising the setting needs a measured bytes-per-slice figure and a container memory limit first.
