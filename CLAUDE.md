...
**`cqlite_splits` stays at 1 because memory binds long before throughput does.** One 15-minute window counted through the seek path holds 6.83 GB of anonymous memory on a single slice with all 16 of its partitions in one merger, and chunking the named keys takes that window to 1.41 GB.  The `backend` container now carries an 8 GB limit, so what raising the setting still waits on is a measured bytes-per-slice figure: N slices build N mergers over the same files.
