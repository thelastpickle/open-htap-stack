...
**This runs in the demo.**&emsp;`demo.drone_latest_status` declares `cdc = true`, and the Sidecar reads the commit log segments Cassandra hard-links into `cdc_raw` and publishes each mutation to `cdc-mutations` as Avro, with the schema in Apicurio.&emsp;Measured: 2,718 records/s, a p50 median of 8.0 s end to end while the publisher is ahead of the writer, no decode failure in 5.4 million records.&emsp;Read that 8.0 s with its rate of 2,000 events a second beside it, because at 400 the same measurement gives 2.2 to 4.7 s; under a sink backlog the publisher becomes the bound and the age reaches minutes.&emsp;The change stream never queries the node, which is the same property the analytical paths have.
