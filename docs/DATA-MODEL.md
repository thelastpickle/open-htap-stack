...
Keyed on the event alone, a question about a period of time had nothing to prune on: `event_time` is
not part of the key, and a partition token is a hash, so "the last fifteen minutes" meant reading
every row on every path. With the window in the partition key it is a question about particular
...
Measured on a seven-core laptop with the ingest running, the same count against a window holding
roughly half a million rows and against a window that holds none:
...
The empty-window column is the pruning, and for the bulk reader it is file skipping rather than a scan
that finds nothing: Spark reported 529,103 records read for the populated window and **0** for the
empty one, across the same seventeen tasks, 16.3 s of executor time against 354 ms.

What the bucket does not buy is CQL as an analytical engine. `count(*)` over a *full* window is about
1.8M rows at the default rate, and it times out at the coordinator — the window is addressable, not
cheap. That is the same division of labour the dashboard's comparison draws, only now the transactional
path can express the question at all.

The shard exists to keep those partitions a sane size. One 15-minute window at the demo's default
2,000 events/s is about 1.8M rows, far too much for a single partition; over sixteen shards it is
nearer 110k rows, a few tens of megabytes. It comes from a hash of the event's id rather than from the
asset, so the spread does not collapse when the fleet is small — and it is a hash rather than the id
taken modulo sixteen, because these are version-1 UUIDs whose low bits are the node field, which is
constant for a host: that arrangement measured as every event landing in one shard.

Both numbers are data model rather than tuning. Change either and existing rows keep the bucket and
shard they were written with, so queries naming the new ones stop matching the old rows.

The bucket is `text`, not `timestamp`, because it is written by hand into queries that four engines
have to parse, and a quoted string means the same thing in CQL, Presto SQL and Spark SQL where a
timestamp literal does not. It sorts lexicographically, so a range predicate still reads naturally on
the paths that cannot prune on it.
