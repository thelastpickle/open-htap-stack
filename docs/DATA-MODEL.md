...
That count costs the backend its own memory, which is what limits how large a question this path can be asked. &emsp;Measured on the Python backend, before the container carried a memory limit: resident memory went from 340.3 MB to 7.224 GB in 26 s and settled at 4.088 GB, and a CI comparison over another closed window took it from 4.274 GB to 11.08 GB, where it stayed after the run. &emsp;An earlier identical count was killed for exceeding memory. &emsp;What bounds it now is the 8 GB cgroup limit `podman-compose.yml` gives the backend, so a merge heading for 11 GB is an out-of-memory kill of that container rather than a slow answer, and the figure to read is `anon` from the cgroup rather than resident size. &emsp;`podman-compose.yml`'s comment on that limit carries the two figures measured against the reactor build: 1.09 GB for a window read one partition at a time, and about 3.9 GB per million rows for the same window read at once. &emsp;Why the seek merger holds that much is not diagnosed.
