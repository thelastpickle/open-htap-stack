...
Where the ten stand now, at `6b74194`. &emsp;Two went with the code: `cqlite_client.py:4` and `models.py:254` no longer exist, and the classes that replaced them were written without the claim rather than translating it. &emsp;Six are restated: `docs/MISSION-CONTROL.md`'s path table and `CLAUDE.md`'s say no snapshot, no Sidecar and no second container, the two frontend pages say no second service, and `Explore.tsx:198` and `:272` name Presto and Spark, which is what "the JVM paths" meant in a sentence about CPU.
...
| `spark_bulk` | 2,508.7, 12,428.0, 16,004.9 ms | a snapshot per read, and the spread is the snapshot rather than the scan |
| `cqlite` | 126.9, 151.1, 171.5 ms beside the other four, 56.3 to 101.3 ms alone | the Python's recorded range for the same table is 54 to 153 ms, so the two overlap and neither is faster on this evidence |
...
| F2 the no-JVM claim | `Remove the Python backend and ingress` restates six of the ten, `CLAUDE.md` and `docs/MISSION-CONTROL.md`'s path tables and the four frontend sites, and the two Python ones went with the code. &emsp;Open for two: `README.md:332` and the heading at `.claude/skills/measure/SKILL.md:78` |
...
Every row is filled. &emsp;Two are open and say what remains: F2's last two places, `README.md:332` and one heading in the `measure` skill, and F11's typo measurement. &emsp;F15 is answered as far as a measurement licenses and no further.
