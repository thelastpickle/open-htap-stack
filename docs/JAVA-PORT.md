...
- [What must be measured rather than carried over](#what-must-be-measured-rather-than-carried-over)
  - [What the port measured](#what-the-port-measured)
...
What the path demonstrates is unchanged: no snapshot, no Sidecar, no coordinator, and the parse and the SQL running inside the dashboard's own process rather than in a separate engine. &emsp;So the distinction to restate is a second service against the dashboard's own process, and not a JVM against no JVM. &emsp;Service rather than container, and one wording at every site: a service is the thing a reader can point at, where whether it runs in a container is how this demo happens to ship it. &emsp;The claim has to be restated rather than quietly dropped, because "no JVM" was never the property; it was one way of saying the property.
...
Where the ten stand now, at `6b74194`. &emsp;Two went with the code: `cqlite_client.py:4` and `models.py:254` no longer exist, and the classes that replaced them were written without the claim rather than translating it. &emsp;Six are restated, all six in the same words, no snapshot, no Sidecar and no second service: `docs/MISSION-CONTROL.md`'s path table, `CLAUDE.md`'s, and the two frontend pages, while `Explore.tsx:198` and `:272` name Presto and Spark, which is what "the JVM paths" meant in a sentence about CPU.

The last two are closed as well, in the commit this paragraph sits in. &emsp;`README.md:332` says no snapshot and no second service, and the `measure` skill's heading names the paths that run in another service where it grouped "the JVM paths"; its body already named Spark and the bulk reader against cqlite in the backend's own process, so only the heading was left. &emsp;An eleventh turned up while they were being swept, in this repository's own Java: `BenchmarkResponse`'s javadoc described the path as having no JVM in a class compiled into one.

One instance stays on purpose. &emsp;`htap-cqlite/dist/cqlite_datafusion.h:5` carries the claim, and that file is a copy of the fork's header rather than this repository's own text; `htap-cqlite/dist/VENDOR.md` already reserves such corrections for the fork, where the copy would otherwise stop describing the commit the libraries were built from.
...
Where that stands now. &emsp;`mvn -o -ntp verify` runs **868 tests over 121 classes** and takes 37.1 s: 60 in `htap-common`, 38 in `htap-cqlite`, 632 in `htap-backend`, 72 in `htap-sink` and 66 in `htap-producer`. &emsp;Nothing in it starts a container, which is the whole argument for having both suites: a wrong shard number fails in well under a minute where the compose suite takes about fifteen to a first failure.
...
- The demo schema, which is not an HTTP shape and is the one item here that breaks every path rather than a page. &emsp;The sink owns it, so porting the sink is what could have moved a key or dropped an option. &emsp;Checked rather than asserted: the Java sink built its fourteen tables into a keyspace of its own beside the running Python sink, and `DESCRIBE KEYSPACE` of the two is 457 lines each with a **zero-line diff** once the keyspace name is normalised, so every key, every option, `transactional_mode` on the same six tables, `cdc` on the same one and the SAI index all agree.
...
That capture-and-diff existed while both backends did, and it went with the Python. &emsp;What protects the shapes now is the dashboard step itself, run against the Java backend: **twenty of its twenty-one sections pass**, extracted verbatim from the workflow through the `ci-step` skill with one substitution, the published port, because an unrelated container on this machine holds host 8000. &emsp;A section covering the six routes no other section reads was added with the switchover, since deleting the comparison step would otherwise have left `/api/zones`, `/api/zones/what-if`, `/api/alerts`, `/api/overview/ingestion-history`, its CSV and `/api/map/polygon-stats` asserted nowhere.

The twenty-first is a wall clock and not the port. &emsp;"The whole event history, grouped" asks for a curl budget of 1,920 s and its own 1,200-second step limit left it 796. &emsp;Run to completion outside the step, that comparison took between 5,444 s and 5,745 s over an `events` table of some 149 million rows and 35 GiB, with cassandra and presto done by 1,112 s, spark by 1,438 s, `spark_bulk` by 4,830 s and cqlite last, sampled every 120 s. &emsp;CI runs that section against a stack minutes old, so what does not fit the budget is this laptop's dataset.
...
Where those stand now. &emsp;The producer's rate, the sink's, and one small question down all five paths are re-measured, and every figure the port produced is in [What the port measured](#what-the-port-measured) below. &emsp;Three of the five rows above are **not** re-measured: the cqlite timings per split and per key chunk, the memory figures with their `anon` guidance, and the Accord medians. &emsp;`CLAUDE.md` still carries the Python's for those three, each quoted with the process it was measured on rather than silently reattributed.

The CDC delay has a reason of its own, and it is worth stating rather than leaving the row blank. &emsp;Publication had stopped at 08:24 UTC before this work began, so `latency_p50_ms` was null in every run the port made and no delay figure comes from it. &emsp;The tail itself is verified: attached to the live topic across its three partitions it decoded 198 records with zero decode failures, taking schema id 1 from Apicurio. &emsp;Why the publisher stopped is a separate matter and is not fixed here.
...
All three are answered. &emsp;`TimeUuids` mints its own v1 and `TimeUuidsTest` holds the distinctness at the producer's cadence, the millisecond round trip and the multicast bit, including the hardest case: 16,000 identifiers minted by eight threads at one instant, all distinct, which is what `Uuids.startOf` fails. &emsp;`Blake2b` is checked against digests `hashlib.blake2b` produced, the RFC 7693 vector among them, so a vector written by the Python and one written by the Java hash a token to the same dimension. &emsp;And the spelling is one class, `Cells`, with the dashboard step's five-path comparison as the assertion that it holds.
...
Two conditions travel with every figure here, and they are why these give an ordering and a magnitude rather than a controlled measurement: the sink was writing about 2,000 rows a second throughout, and up to three other builds were compiling on the same seven-core machine. &emsp;Each row says what else was true when it was taken.

| What | Figure | Conditions |
| --- | --- | --- |
| the reactor's suite | 868 tests over 121 classes, 37.1 s | `mvn -o -ntp verify`; nothing in it starts a container |
| the producer's send loop | 0.92 to 1.01 µs an event, about 1.0 million a second | three runs of 200,000 with no broker, so the model step, the identifier and the JSON alone |
| the producer, sustained | 1,971 to 1,991 records a second by its own count, 1,913 by the broker's | target 2,000, 100 assets, the settings poll on; the broker's figure is 243,000 records of end offsets over 127 s, taken without asking the producer |
| the Python sink on those records | 1,637 to 1,942 a second, no error | the same window, and the check the porting order was chosen for: the sink was ported first and tested against the Python producer, so neither port was ever checked only against itself |
| the Java sink, draining a backlog | 6,559 writes a second, 1,180,641 rows in 180 s | a fresh group id reading the topic from its start, while the same node took the Python sink's 1,064 to 1,438 a second and served a whole-history comparison. &emsp;Not comparable with the Python sink's 3,148 in `CLAUDE.md`, which was draining its own backlog on a quieter node and is a different job |
| the schema the sink owns | 457 lines each, a zero-line diff | `DESCRIBE KEYSPACE` of the Java sink's own keyspace against the Python sink's `demo` |
| the backend's start | Quarkus 1.361 s idle and 3.243 s under load; the packaged jar answers `/api/health` 0.980 s after `java -jar` | the 0.980 s is one run on a port of its own with no engine reachable, so it is the HTTP layer and not a warm stack |
| a `backend` image rebuild | 14 s unchanged, 41 s for one Java file, 2 min 59 s for a `pom.xml`, 3 min 26 s cold | the sink's are 24 s and 2 min 2 s. &emsp;`dependency:go-offline` re-resolves the module's whole reactor slice, so a dependency change costs nearly a cold build where a code change costs a compile |
| the SQL console | 17 statements in 2,085.66 ms, a reset of 25 in 1,439.85 ms, eight warm `/tables` batches in 279.85 to 851.88 ms | live `accord-sql`. &emsp;A later reset of the same 25 statements took 15,144.26 ms with the node compacting, so read the first two as one run rather than as a range |
| the producer's shutdown | `podman stop` returns in 0.72, 0.77 and 0.77 s | the hook interrupts the send loop and joins the thread that closes the client, where podman otherwise waits out its own ten seconds.  A single earlier sample read 0.896 s on the code whose close force-closed rather than flushed, so the figure does not tell the two paths apart: a flush of one 20 ms linger window costs less than podman's own stop |

**A JVM has a first request, so a figure taken from a freshly started backend is a figure about the warm-up.** &emsp;Measured after a container restart, `/api/map/live` took 799.7 ms on the first request and 49.8 ms on the second, then 15 to 57 ms over twenty-two more with no further trend: about sixteen times a warm request, and only the first one. &emsp;The cqlite path pays a smaller one for a different reason, 165.0 ms on the first read of `drone_latest_status` against 56.3 to 101.3 ms on the six after it, which is the Panama binding's first downcall and the library's first registration rather than the JIT. &emsp;Where this matters in practice is CI, whose first call to a route is often its only call.

**One small question down all five paths**, three sequential runs through the Java backend of `SELECT event_type, count(*) AS assets FROM drone_latest_status GROUP BY event_type ORDER BY event_type LIMIT 5`:

| Path | Three runs | What it says |
| --- | --- | --- |
| `cassandra` | refuses in 7.5, 8.3 and 13.1 ms | the refusal is the finding, in the node's own words: "Group by is currently only supported on the columns of the PRIMARY KEY, got event_type" |
| `presto` | 491.2, 535.0, 581.9 ms | |
| `spark` | 842.1, 2,306.4, 2,653.0 ms | |
| `spark_bulk` | 2,508.7, 12,428.0, 16,004.9 ms | a snapshot per read, and the run reports what its own snapshot cost.  Four later runs of the same statement, each taking a fresh one: 8,037.7 ms of which 2,113.2 was the snapshot, 36,235.1 of 1,822.3, 22,643.1 of 12,212.9 and 24,856.8 of 3,927.8, so the share ranges from 5% to 54% and the run itself swings by a factor of four on a table of 146,554 bytes.  Read the share as unsettled: the `measure` skill's rule of roughly a quarter sits inside that range, and on this machine the run-to-run variation is larger than the thing being attributed |
| `cqlite` | 126.9, 151.1, 171.5 ms beside the other four, 56.3 to 101.3 ms alone | the Python's recorded range for the same table is 54 to 153 ms, at `frontend/src/pages/Explore.tsx`, and it was taken on a drained sink where these were taken with the sink writing about 2,000 rows a second.  On the alone column the two overlap, so neither is faster on this evidence, and the condition difference is why that is the column to compare |

That last row is the one a reader of this document will want, and it is the one to state carefully: the port did not make the cqlite path faster or slower by any margin this stack can resolve.
...
| F2 the no-JVM claim | `Remove the Python backend and ingress` restates six of the ten, `CLAUDE.md` and `docs/MISSION-CONTROL.md`'s path tables and the four frontend sites, and the two Python ones went with the code. &emsp;`Say what the five paths are, and record what the port measured` closes the last two, the README's OLAP-path bullet and the heading in the `measure` skill, and an eleventh found while sweeping: `BenchmarkResponse`'s own javadoc. &emsp;Two more turned up in the reactor's own text, `htap-cqlite`'s POM description and `CqlitePath`'s javadoc, and were swept with them, so thirteen sites say the same words: no snapshot, no Sidecar and no second service. &emsp;One instance stays on purpose, in `htap-cqlite/dist/cqlite_datafusion.h`, which is a copy of the fork's header |
...
Every row is filled, and one is open: F11's typo measurement, which nobody has made. &emsp;F15 is answered as far as a measurement licenses and no further.
