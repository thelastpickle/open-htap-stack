...
| the producer's shutdown | `podman stop` returns in 0.896 s | the hook interrupts the send loop and joins it, where podman otherwise waits out its own ten seconds |
...
| `spark_bulk` | 2,508.7, 12,428.0, 16,004.9 ms | a snapshot per read.  What the snapshot costs was separated afterwards with `reuse_snapshot`: a fresh one took 16,902.5 ms and the two reads that reused it 6,757.0 and 8,921.3 ms, so the snapshot is the larger part of a run on this small table.  The 2,164 ms between the two reused runs is not the snapshot, and the concurrent load is the candidate rather than a cause established here |
