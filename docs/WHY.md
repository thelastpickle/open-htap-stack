...
DiskANN, developed by Microsoft Research, was designed specifically for this regime. &emsp;It combines two things. &emsp;The first is the Vamana graph algorithm: it gives each node a higher out-degree than HNSW's multi-layer structure, and its pruning keeps long-range edges among that node's neighbours, so a search needs fewer hops. &emsp;The second is product quantisation (PQ) compression of the vectors themselves. &emsp;The compressed vectors stay in memory; the full-precision vectors live on disk.
