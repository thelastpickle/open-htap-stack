...
/**
 * How events are partitioned, as the backend reports it: a window of so many
 * minutes, spread over so many shards.  Read from the stack rather than computed
 * here, because the buckets exist only because the sink wrote them, so it is the
 * stack's clock and configuration that decide which ones can be named.
 */
...
const COMPARE_PRESETS = [
  ...
  {
    key: 'window',
    label: 'One closed window',
    cost: 'seconds',
    hint: 'The same grouping, bounded to the partitions holding one finished window — the question the data model was shaped for',
    ...
    build: (bucket: EventWindow) =>
      'SELECT event_type, count(*) AS event_count,\n' +
      '       min(temp_internal_c) AS coldest, max(temp_internal_c) AS hottest\n' +
      'FROM events\n' +
      "WHERE event_bucket = '" + bucket.last_complete + "'\n" +
      '  AND shard IN (' + shardList(bucket.shards) + ')\n' +
      'GROUP BY event_type\n' +
      'ORDER BY event_type\n' +
      'LIMIT 5',
  },
        ...
        <>
          ...
          <div className="glass-panel space-y-4 rounded-xl border border-white/5 p-6">
            ...
            <div className="text-on-surface-variant space-y-3 border-t border-white/5 pt-4 text-xs leading-relaxed">
              ...
              <p>
                <strong className="font-bold">One closed window is the same question, bounded.</strong>{' '}
                Events are partitioned by a 15-minute window and a shard, so naming the window names
                the partitions that hold it: Cassandra reads them and needs no ALLOW FILTERING, Presto
                and the connector push the predicate down, and the bulk reader turns it into a
                partition-key lookup instead of a scan. Compare it against the whole history above and
                the difference is the data model, not the engines. Two things follow. Cassandra still
                declines the grouping, because a bounded question is not the same as an expressible
                one. And a finished window cannot change, so the paths that can answer it agree on the
                totals exactly, where the unbounded presets see the table grow underneath them.
              </p>
              <p>
