...
# something that fails quietly, hours later, or in a way that reads as corruption.  Nor does
# it check for a JDK, because the three Maven modules are built inside the image by
# maven:3.9-eclipse-temurin-25 and the host needs no Java to run the stack; JDK 25 is needed
# only to run the reactor yourself, with `mvn -B -ntp -C verify`.  The images are also
# published, one per service, at ghcr.io/thelastpickle/open-htap-stack/<service>:trunk-java,
# for anyone who would rather not build; :latest is the default branch's, not this one's.
#
...
# ── 1. The architecture ──────────────────────────────────────────────────────────────
# The backend image installs a prebuilt cqlite shared library chosen by `uname -m`, and
# there is a library for two architectures only.  Nothing later in this script can substitute
...
if [ -n "$READER_ARCH" ]; then
  pass "architecture $ARCH has a cqlite library"
else
  fail "architecture $ARCH has no cqlite library" \
       "the stack runs on x86_64 and on arm64 only; see htap-cqlite/dist/VENDOR.md to build a third"
fi
...
# failure and not a warning: the Sidecar tarball, the cqlite library for this architecture
# and the Spark offline ivy jars are each read by a build here.
...
$(ls htap-cqlite/dist/*linux-${READER_ARCH}.so.gz 2>/dev/null)"
  CANDIDATES="${CANDIDATES}
$(find spark/ivy/cache -name '*.jar' 2>/dev/null)"
  MISSING_ARTEFACT="no"
  [ -n "$READER_ARCH" ] && [ -z "$(ls htap-cqlite/dist/*linux-${READER_ARCH}.so.gz 2>/dev/null)" ] && MISSING_ARTEFACT="yes"
  ...
  if [ "$MISSING_ARTEFACT" = "yes" ]; then
    fail "no cqlite library for linux-${READER_ARCH} in htap-cqlite/dist/" \
         "the checkout is incomplete or predates this architecture; see htap-cqlite/dist/VENDOR.md"
